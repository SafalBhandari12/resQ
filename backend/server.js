...
if (!fs.existsSync(CSV_FILE)) {
  fs.writeFileSync(CSV_FILE, 'id,image_filename,latitude,longitude,location,description,status\n');
}
  ...
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      headers.forEach((header, index) => {
        ...
        if (header === 'latitude' || header === 'longitude' || header === 'id') {
...
app.put('/admin/report/:report_id/status', (req, res) => {
  try {
    ...
    const updatedReport = updatedLines.find(line => line.startsWith(reportId + ','));
...
app.post('/login', (req, res) => {
  try {
    ...
    const contacts = users.filter(u => u.mobile_number !== mobile_number).map(u => ({
